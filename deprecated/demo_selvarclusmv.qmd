...
+ Check Maugis with 
  ...
  - missing values: Fail - But better result
Results for 5% missing values:
mixall:
Num clusters: 4 
Correctly identified relevant variables: 3, 2 
Misclassified variables: 6, 1 

+ Testing SelVarMix package: In progress (Package)
...
# Incorporate missing values and mixed data treatment (SR)
## Final Version
```{r}
...
<!-- ## Parallel version - extensive missing values treatment -->
<!-- ```{r} -->
...
<!--   # Validate missing_handling parameter -->
<!--   if(!missing_handling %in% c("impute", "partial_cases")) -->
<!--     stop("missing_handling must be either 'impute' or 'partial_cases'") -->
...
<!--   # Function to handle missing values using imputation -->
<!--   handle_missing_values <- function(model, data) { -->
<!--     if(missing_handling == "impute") { -->
<!--       # Get imputed values from the model -->
<!--       imp_values <- missingValues(model) -->
<!--       if(!is.null(imp_values) && nrow(imp_values) > 0) { -->
<!--         # Convert imputed values to a more usable format -->
<!--         imp_mat <- as.matrix(data) -->
<!--         for(i in 1:nrow(imp_values)) { -->
<!--           imp_mat[imp_values$row[i], imp_values$col[i]] <- imp_values$value[i] -->
<!--         } -->
<!--         return(as.data.frame(imp_mat)) -->
<!--       } -->
<!--     } -->
<!--     return(data) -->
<!--   } -->

<!--   # Function to get relevant complete cases based on approach -->
<!--   get_relevant_cases <- function(data, var_idx = NULL) { -->
<!--     if(missing_handling == "partial_cases") { -->
<!--       if(is.null(var_idx)) { -->
<!--         # For the clustering step, use all available data -->
<!--         return(1:nrow(data)) -->
<!--       } else { -->
<!--         # For regression, only remove rows with NA in relevant variables -->
<!--         relevant_cols <- c(var_idx, setdiff(1:ncol(data), var_idx)) -->
<!--         return(which(complete.cases(data[, relevant_cols, drop = FALSE]))) -->
<!--       } -->
<!--     } else { -->
<!--       # For imputation approach, use all cases -->
<!--       return(1:nrow(data)) -->
<!--     } -->
<!--   } -->

<!--   # Modified BIC computation for partial cases -->
<!--   compute_modified_bic <- function(bic_value, n_full, n_used) { -->
<!--     if(missing_handling == "partial_cases") { -->
<!--       # Adjust BIC for reduced sample size -->
<!--       return(bic_value * (n_used/n_full)) -->
<!--     } -->
<!--     return(bic_value) -->
<!--   } -->
...
<!--   # Modified fit_cluster_model to handle missing values -->
<!--   fit_cluster_model <- function(data, G) { -->
<!--     tryCatch({ -->
<!--       prepared <- prepare_data_and_cluster_function(data) -->
<!--       if (prepared$is_mixed) { -->
...
<!--       # Handle missing values if using imputation -->
<!--       if(missing_handling == "impute") { -->
<!--         data <- handle_missing_values(model, data) -->
<!--       } -->

<!--       return(list(model = model,  -->
<!--                  BIC = -model@criterion,  -->
<!--                  nbCluster = model@nbCluster, -->
<!--                  imputed_data = data)) -->
<!--     }, error = function(e) { -->
<!--       return(list(model = NULL, BIC = -Inf, nbCluster = NA, imputed_data = NULL)) -->
<!--     }) -->
<!--   } -->
...
<!--   # Export necessary functions and objects to the cluster -->
<!--   clusterExport(cl, c("prepare_data_and_cluster_function", "get_data_type",  -->
<!--                       "fit_cluster_model", "compute_bic_reg", "strategy", -->
<!--                       "handle_missing_values", "get_relevant_cases", -->
<!--                       "compute_modified_bic", "missing_handling"),  -->
<!--                 envir = environment()) -->
...
<!--       # Get relevant cases based on approach -->
<!--       relevant_cases <- get_relevant_cases(S_minus_i, i) -->
<!--       S_minus_i_obs <- S_minus_i[relevant_cases, , drop = FALSE] -->
<!--       S_i_obs <- S[relevant_cases, i, drop = FALSE] -->

<!--       # Compute BIC with adjustment if needed -->
<!--       BICreg <- compute_bic_reg(S_minus_i_obs, S_i_obs, -->
<!--                                family = get_data_type(S[, i])) -->
<!--       BICreg <- compute_modified_bic(BICreg, nrow(S), length(relevant_cases)) -->

<!--       list(BIC_total = model_minus_i$BIC + BICreg,  -->
<!--            BICreg = BICreg, -->
<!--            model = model_minus_i$model,  -->
<!--            nbCluster = model_minus_i$nbCluster, -->
<!--            n_used = length(relevant_cases), -->
<!--            imputed_data = model_minus_i$imputed_data) -->
<!--     }) -->
...
<!--         Model = class(remove_results[[arg_remove]]$model)[1], -->
<!--         G = remove_results[[arg_remove]]$nbCluster, -->
<!--         N_used = remove_results[[arg_remove]]$n_used, -->
<!--         stringsAsFactors = FALSE -->
<!--       )) -->
<!--       NS[[removed_var]] <- S[[removed_var]] -->
<!--       S <- S[, -arg_remove, drop = FALSE] -->
...
<!--       if(missing_handling == "impute") { -->
<!--         S <- remove_results[[arg_remove]]$imputed_data -->
<!--       } -->
<!--     } else { -->
<!--       info <- rbind(info, data.frame( -->
<!--         Var = colnames(S)[arg_remove], BIC = BICS, BICdiff = m_remove, -->
...
<!--         # Get relevant cases based on approach -->
<!--         relevant_cases <- get_relevant_cases(S, i) -->
<!--         S_obs <- S[relevant_cases, , drop = FALSE] -->
<!--         NS_i_obs <- NS[relevant_cases, i, drop = FALSE] -->
...
<!--              nbCluster = model_plus_i$nbCluster, -->
<!--              n_used = length(relevant_cases), -->
<!--              imputed_data = model_plus_i$imputed_data) -->
<!--       }) -->
...
<!--           Model = class(add_results[[arg_add]]$model)[1], -->
<!--           G = add_results[[arg_add]]$nbCluster, -->
<!--           N_used = add_results[[arg_add]]$n_used, -->
<!--           stringsAsFactors = FALSE -->
<!--         )) -->
<!--         S[[added_var]] <- NS[[added_var]] -->
<!--         NS <- NS[, -arg_add, drop = FALSE] -->
...
<!--         if(missing_handling == "impute") { -->
<!--           S <- add_results[[arg_add]]$imputed_data -->
<!--         } -->
<!--       } else { -->
<!--         info <- rbind(info, data.frame( -->
<!--           Var = colnames(NS)[arg_add], BIC = BIC_add[arg_add], BICdiff = m_add, -->
...
<!--     if(verbose) { -->
<!--       print(info[nrow(info), c("Var", "BICdiff", "Step", "Decision", "N_used")]) -->
<!--     } -->
...
<!--     variables = varnames, -->
<!--     subset = subset, -->
<!--     cluster_model = final_model$model, -->
<!--     steps.info = info, -->
<!--     optimal_G = optimal_G, -->
<!--     search = "greedy", -->
<!--     direction = "backward", -->
<!--     missing_handling = missing_handling, -->
<!--     imputed_data = if(missing_handling == "impute") final_model$imputed_data else NULL) -->
<!--   class(out) <- "clustvarsel_mixall" -->
<!--   return(out) -->
<!-- } -->
<!-- ``` -->
