...
# Set up cores 
total_cores <- detectCores(logical = TRUE)
nbcores_to_use <- min(6, total_cores - 1)  # 12 cores 
cl <- makeCluster(nbcores_to_use)
...
# Create a combined list of (run, dataset) pairs
tasks <- expand.grid(run_idx = 1:n_runs, dat_idx = 1:length(data_sen))

# Parallel simulation over combined tasks
timing_results_list <- foreach(task = 1:nrow(tasks), .combine = rbind, 
                               .packages = c("missRanger", "clustvarsel", 
                                             "doParallel", "foreach", 
                                             "stringr", "dplyr",
                                             "SelvarMix", "aricode",
                                             "mclust"
                                             )) %dopar% {
  run_idx <- tasks$run_idx[task]
  dat_idx <- tasks$dat_idx[task]
  
  # Initialize timing results for this task
  task_timing_results <- data.frame(
    Dataset = character(),
    Missing_Rate = numeric(),
    Model = character(),
    Run = numeric(),
  ...
  cat("\n=== Starting Run", run_idx, "for Dataset", dat_idx, "===\n")
  ...
  for (miss_idx in seq_along(missing_rates)) {
    ...
    for (m in seq_along(models)) {
      ...
      tryCatch({
        # Produce missing data if needed
        if (miss_rate == 0) {
          data_missing <- current_data
        } else {
          data_missing <- produce_NA(current_data, 
        ...
        # Timing info for this run/model
        task_timing_results <- rbind(task_timing_results, data.frame(
          Dataset = dataset_name,
          Missing_Rate = miss_rate,
          Model = model_name,
          Run = run_idx,
  ...
  task_time <- as.numeric(difftime(Sys.time(), task_start_time, units = "secs"))
  cat("=== Completed Run", run_idx, "for Dataset", dat_idx, "in", round(task_time, 2), "seconds ===\n")
  
  # Return timing results collected during this task
  task_timing_results
}
...
# Combine all timing results from parallel tasks
timing_results <- timing_results_list

total_time <- as.numeric(difftime(Sys.time(), total_start_time, units = "secs"))
cat("\nTotal simulation time:", round(total_time, 2), "seconds\n")

# Post-Processing and Aggregation
# Since results are saved as individual RDS files, you can read them as follows:
...
# Apply the transformation and aggregation
results_tibble <- results_tibble %>%
  mutate(
    Correct_Variables = map2_int(Dataset, Relevant_Variables, ~ {
      true_vars <- true_S[[as.character(.x)]]
      selected_vars <- str_split(as.character(.y), ",\\s*")[[1]]
      if(length(selected_vars) != length(true_vars)) {
        return(0)
      } else {
  ...
  mutate(
    Correct_Clusters = mapply(function(dataset, clusters) {
      clusters == true_num_clusters[[as.character(dataset)]]
    }, Dataset, Num_Clusters)
  )
