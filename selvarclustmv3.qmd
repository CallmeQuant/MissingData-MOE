...
Progress:
-10-12-2024
 + No missing values handle => Ranking fails
 + If variability not large, scaling no too affected

-2-1-2024
 + Sensitivity runs time: 17.39 hours
 + Average time per run: 52.18 minutes
...
# Structure: Dataset -> MissingRate -> Model -> Run -> Result
sensitivity_results <- vector("list", length(data_sen))
names(sensitivity_results) <- paste0("Dataset_", seq_along(data_sen))

for(i in seq_along(data_sen)) {
  sensitivity_results[[i]] <- vector("list", length(missing_rates))
  names(sensitivity_results[[i]]) <- paste0("MissingRate_", missing_rates)
  
  for(j in seq_along(missing_rates)) {
    sensitivity_results[[i]][[j]] <- vector("list", length(models))
    names(sensitivity_results[[i]][[j]]) <- models
    
    for(m in seq_along(models)) {
      sensitivity_results[[i]][[j]][[m]] <- vector("list", n_runs)
      names(sensitivity_results[[i]][[j]][[m]]) <- paste0("Run_", 1:n_runs)
    }
  }
}

# Initialize the timing results dataframe
# Structure: Dataset, Missing_Rate, Model, Run, Run_Time_Seconds
timing_results <- data.frame(
  Dataset = character(),
  Missing_Rate = numeric(),
  Model = character(),
...
nbcores_to_use <- min(6, total_cores - 1)
cat("Using", nbcores_to_use, "cores for computation\n")
...
for (run_idx in 1:n_runs) {
  ...
  for (dat_idx in seq_along(data_sen)) {
    ...
    cat("\nProcessing Dataset", dat_idx, "with", num_vars, "variables. Fixed hsize =", fixed_hsize, "\n")
    ...
    for (miss_idx in seq_along(missing_rates)) {
      ...
      for (m in seq_along(models)) {
        ...
        # Define a unique identifier for the result file
        ...
        if (file.exists(result_file)) {
          cat("      Loading existing results for Run", run_idx, "Model", model_name, "\n")
          sensitivity_results[[dat_idx]][[miss_idx]][[model_name]][[run_idx]] <- readRDS(result_file)
          next
        }
        ...
        tryCatch({
          ...
          if (model_name == "SelvarClustLasso") {
            ...
            selvar_time <- as.numeric(difftime(Sys.time(), 
                                               selvar_start_time, units = "secs"))
            
            # Extract results and compute metrics
            imputed_data <- selvar_result$imputedData
            predicted_labels <- selvar_result$partition
            
            ari <- tryCatch({
              adjustedRandIndex(current_labels, predicted_labels)
            }, error = function(e) {
              cat("        Warning: Failed to compute ARI for SelvarClustLasso:", conditionMessage(e), "\n")
              return(NA)
            })
            ...
            nrmse <- tryCatch({
              ...
              cat("        Warning: Failed to compute NRMSE for SelvarClustLasso:", conditionMessage(e), "\n")
              return(NA)
            })
            ...
            w_nrmse <- tryCatch({
              ...
              cat("        Warning: Failed to compute Weighted NRMSE for SelvarClustLasso:", conditionMessage(e), "\n")
              return(NA)
            })
            ...
            ciie <- tryCatch({
              ...
              cat("        Warning: Failed to compute CIIE for SelvarClustLasso:", conditionMessage(e), "\n")
              return(NA)
            })
            ...
            result <- data.frame(
              ...
              Relevant_Variables = paste(selvar_result$S, collapse = ", "),
              ARI = round(ari, 4),
              Run_Time_Seconds = round(selvar_time, 2),
              stringsAsFactors = FALSE
            )
            ...
            clust_time <- as.numeric(difftime(Sys.time(), 
                                              clust_start_time, units = "secs"))
            
            # Extract results and compute metrics
            imputed_data <- data_imputed
            predicted_labels <- clust_result$model$classification
            
            ari <- tryCatch({
              adjustedRandIndex(current_labels, predicted_labels)
            }, error = function(e) {
              cat("        Warning: Failed to compute ARI for Clustvarsel:", 
                  conditionMessage(e), "\n")
              return(NA)
            })
            ...
            nrmse <- tryCatch({
              ...
              cat("        Warning: Failed to compute NRMSE for Clustvarsel:", 
                  conditionMessage(e), "\n")
              return(NA)
            })
            ...
            w_nrmse <- tryCatch({
              ...
              cat("        Warning: Failed to compute Weighted NRMSE for Clustvarsel:", 
                  conditionMessage(e), "\n")
              return(NA)
            })
            ...
            ciie <- tryCatch({
              ...
              cat("        Warning: Failed to compute CIIE for Clustvarsel:", 
                  conditionMessage(e), "\n")
              return(NA)
            })
            ...
            result <- data.frame(
              ...
              WNRMSE = round(w_nrmse, 4),
              CIIE = round(ciie, 4),
              BIC = clust_result$model$bic,
              Num_Clusters = clust_result$model$G,  
              Relevant_Variables = paste(clust_result$subset, collapse = ", "),
              ARI = round(ari, 4),
              Run_Time_Seconds = round(clust_time, 2),
              stringsAsFactors = FALSE
            )
          }
          
          # Calculate elapsed time for this configuration
          config_time <- as.numeric(difftime(Sys.time(), config_start_time, units = "secs"))
          ...
          # Store the result in the nested list
          sensitivity_results[[dat_idx]][[miss_idx]][[model_name]][[run_idx]] <- result
          
          # Save intermediate results for this run and model
          saveRDS(result, result_file)
          cat("      Saved results for Run", run_idx, "Model", model_name, "\n")
          print(result)
          cat("      Completed in", round(result$Run_Time_Seconds, 2), "seconds\n")
          
          # Cleanup to free memory
          rm(data_missing, selvar_result, clust_result, imputed_data, 
             predicted_labels, result)
          gc()
          
        }, error = function(e) {
          cat("\n        Error occurred for Model =", model_name, "in Dataset", dat_idx, "hsize =", fixed_hsize, "Missing Rate =", miss_rate, "Run", run_idx, ":\n")
          cat("        Error message:", conditionMessage(e), "\n")
        })
      }
    }
...
total_time <- as.numeric(difftime(Sys.time(), total_start_time, units = "secs"))
cat("\nTotal simulation time:", round(total_time, 2), "seconds\n")
...
transform_sensitivity_results <- function(sensitivity_results, 
  ...
  for(dat_idx in seq_along(sensitivity_results)) {
    ...
    for(miss_idx in seq_along(dataset_results)) {
      ...
      for(m in names(missing_rate_results)) {
        model_results <- missing_rate_results[[m]]
        
        # Loop through each run
        for(run in seq_along(model_results)) {
          result <- model_results[[run]]
  ...
  # Combine into a single tibble
  results_tibble <- bind_rows(all_results) |>
    mutate(across(c(Dataset, Model, Method, Relevant_Variables), as.factor)) |>
    mutate(across(c(Num_Variables, Missing_Rate, HSize, 
                   NRMSE, WNRMSE, CIIE, BIC, 
                   Num_Clusters, ARI, Run_Time_Seconds), as.numeric))
  
  # Calculate Correct Variables
  results_tibble <- results_tibble |>
    mutate(
      Correct_Variables = map2_int(Dataset, Relevant_Variables, ~ {
        true_vars <- true_relevant_vars[[.x]]
  ...
  freq_cluster <- results_tibble |>
    mutate(Correct_Clusters = ifelse(Num_Clusters == true_num_cluster, 1, 0)) |>
    group_by(Dataset, Missing_Rate, Model, HSize, Method) |>
    summarise(
      Avg_NRMSE = mean(NRMSE, na.rm = TRUE),
      SD_NRMSE = sd(NRMSE, na.rm = TRUE),
    ...
    arrange(Dataset, Missing_Rate, Model, HSize)
  
  return(freq_cluster)
}

# Transform and aggregate the results
results_tibble <- transform_sensitivity_results(
  sensitivity_results, 
  timing_results,
  n_runs
)
...
saveRDS(results_tibble, 
        file = sprintf("sensitivity_results/aggregated_results_%d_runs.rds", 
                       n_runs))
...
# Initialize sensitivity_results
sensitivity_results <- vector("list", length(data_sen))
names(sensitivity_results) <- paste0("Dataset_", seq_along(data_sen))
for(i in seq_along(data_sen)) {
  sensitivity_results[[i]] <- vector("list", length(missing_rates))
...
for (run_idx in 1:n_runs) {
  ...
  for (dat_idx in seq_along(data_sen)) {
    ...
    cat("\nProcessing", dataset_name, "with", num_vars, "variables. Fixed hsize =", fixed_hsize, "\n")
      ...
      for (m in seq_along(models)) {
        ...
        tryCatch({
          ...
          if (model_name == "SelvarClustLasso") {
            ...
            selvar_time <- as.numeric(difftime(Sys.time(), selvar_start_time, units = "secs"))
            ...
            clust_time <- as.numeric(difftime(Sys.time(), clust_start_time, units = "secs"))
          ...
          rm(data_missing, selvar_result, clust_result, data_imputed, imputed_data, predicted_labels, result)
          gc()
          
        }, error = function(e) {
          cat("      Error in Run", run_idx, "Model", model_name, ":", conditionMessage(e), "\n")
        })
      }
    }
  }
...
transform_sensitivity_results <- function(sensitivity_results, 
  ...
  for(dat_idx in seq_along(sensitivity_results)) {
    dataset_results <- sensitivity_results[[dat_idx]]
    dataset_name <- names(sensitivity_results)[dat_idx]
    for(miss_idx in seq_along(dataset_results)) {
      missing_rate_results <- dataset_results[[miss_idx]]
      for(m in names(missing_rate_results)) {
        model_results <- missing_rate_results[[m]]
  ...
  results_tibble <- bind_rows(all_results) |>
    ...
    mutate(
      Correct_Variables = map2_int(Dataset, Relevant_Variables, ~ {
        true_vars <- true_relevant_vars[[as.character(.x)]]
        selected_vars <- str_split(as.character(.y), ",\\s*")[[1]]
        if(length(selected_vars) != length(true_vars)) {
          return(0)
        } else {
          return(all(sort(selected_vars) == sort(true_vars)) * 1)
    ...
    mutate(
      Correct_Clusters = mapply(function(dataset, clusters) {
        clusters == true_num_clusters[[dataset]]
...
saveRDS(results_tibble, file = "sensitivity_results/aggregated_results_20_runs.rds")
write.csv(results_tibble, file = "sensitivity_results/aggregated_results_20_runs.csv", row.names = FALSE)
