transform_mixall_imputed <- function(missing_data, missing_vals){
  x_imputed_final <- missing_data
  if (!is.null(missing_vals) && is.matrix(missing_vals) && ncol(missing_vals) == 3) {
    for (j in 1:nrow(missing_vals)) {
      r <- missing_vals[j, 1]  # row
      c <- missing_vals[j, 2]  # col
      v <- missing_vals[j, 3]  # value
      
      # Check bounds before assigning
      if (r > 0 && r <= nrow(x_imputed_final) && 
          c > 0 && c <= ncol(x_imputed_final)) {
        x_imputed_final[r, c] <- v
      }
    }
  }
