...
EM_Gaussian <- function(YNA, K, mecha, diag, rmax=100, 
                        init, stop="classical", tol=0.0001, samplesize=NULL){
  ...
  while(((loglik_res$loglik_obs-prec)>tol) & (r<rmax)){
    ...
    for (k in 1:K){
      for (i in 1:n){
        ...
        if (length(indmiss)>0){
          if(length(indobs>1)){
            if(abs(det(data.matrix(sigma.new[[k]][indobs, indobs])))<=10^(-15)){
              return("error: determinant of a sub-matrix of the covariance matrix is null")
            }
          }else{
            if(abs(sigma.new[[k]][indobs, indobs])<=10^-15){
              return("error: determinant of a sub-matrix of the covariance matrix is null")
            }
          }
          mu.tilde <- mu.new[[k]][indmiss] + sigma.new[[k]][indmiss,indobs]%*%solve(sigma.new[[k]][indobs,indobs])%*%(YNA[i,indobs]-mu.new[[k]][indobs])
  ...
  imputed_data <- YNA
  for (i in 1:n) {
    if (sum(C[i,]) > 0) { # If observation has missing values
      indobs <- which(C[i,] == 0)
      indmiss <- which(C[i,] == 1)
      
      # For each missing position, compute the weighted conditional expectation
      for (j in indmiss) {
        imputed_value <- 0
        for (k in 1:K) {
          # Compute conditional mean for this cluster
          conditional_mean <- mu.new[[k]][j]
          
          # If we have observed values, adjust the conditional mean
          if (length(indobs) > 0) {
            conditional_mean <- mu.new[[k]][j] + 
              sigma.new[[k]][j, indobs] %*% 
              solve(sigma.new[[k]][indobs, indobs]) %*% 
              (YNA[i, indobs] - mu.new[[k]][indobs])
          }
          
          # Weight by posterior probability
          imputed_value <- imputed_value + conditional_mean * tik[i, k]
        }
  ...
  return(list(pik=pi.new, mu=mu.new, sigma=sigma.new, alpha=alpha.new, 
              beta=matrix(0,nrow=K,ncol=d), loglik_vec=loglik_vec, 
              prec=prec, error=error, 
